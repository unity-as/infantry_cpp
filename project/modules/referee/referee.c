...
static Serial_Instance *referee_serial = NULL;
static Daemon_Instance *referee_daemon = NULL;
static referee_info_t referee_info;
static uint8_t referee_rx_buf[256];
...
static void Referee_ParseFrame(uint8_t *buf, uint16_t len)
{
    ...
    {
        xFrameHeader *header = (xFrameHeader *)(buf + offset);
        ...
        uint16_t actual_crc16 = *(uint16_t *)(buf + offset + frame_len - REFEREE_FRAME_TAIL_LEN);
        ...
        // cmd_id 和数据指针
        uint16_t cmd_id = *(uint16_t *)(buf + offset + REFEREE_FRAME_HEADER_LEN);
...
{
    Referee_ParseFrame(referee_serial->recv_buf, len);
}
...
referee_info_t *Referee_Init(UART_HandleTypeDef *huart)
{
    ...
    referee_daemon = Daemon_Register(&daemon_config);
    ...
    referee_serial = Serial_Register(&serial_config);
...
{
    return referee_daemon != NULL && referee_daemon->online;
}
...
void Referee_SendFrame(uint16_t cmd_id, uint8_t *data, uint16_t len)
{
    ...
    // 发送
    Serial_Send(referee_serial, buf, frame_len);
}
...
void Referee_SendInteractive(uint16_t sub_cmd_id, uint16_t receiver_id, uint8_t *data, uint16_t len)
{
    ...
    // 交互数据头
    ext_student_interactive_header_data_t *header = (ext_student_interactive_header_data_t *)buf;
    header->data_cmd_id = sub_cmd_id;
    header->sender_ID = referee_info.id.robot_id;
    header->receiver_ID = receiver_id;
    offset += Interactive_Header_Len;
                    ...
                    uint8_t layer, uint8_t color, ...)
{
    ...
    Referee_SendInteractive(UI_Draw1, referee_info.id.client_id, (uint8_t *)&figure, sizeof(figure));
}
                    ...
                    uint16_t x, uint16_t y, const char *fmt, ...)
{
    ...
    // 图形配置 (15 字节)
    interaction_figure_t *figure = (interaction_figure_t *)buf;
    memcpy(figure->figure_name, graph_name, 3);
    figure->operate_type = operate;

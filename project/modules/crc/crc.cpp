/**
 * @file    crc.cpp
 * @brief   CRC 校验模块实现（C → C++）
 * @note    原 C 版逻辑不变，仅改名 .c→.cpp、去 extern "C"。
 */
#include "crc.h"
...
/*============================================
 * CRC-8/MAXIM (裁判系统帧头校验)
 * 多项式: x^8 + x^5 + x^4 + 1 = 0x31 (反射 0x8C)
 * 初始值: 0xFF
 * 输入/输出均反射 (LSB-first)
 ============================================*/
...
uint8_t CRC8_Calculate(const uint8_t* data, uint16_t len)
{
    ...
    while (len--)
    {
        ...
        for (uint8_t i = 0; i < 8; i++)
            crc = (crc & 0x01) ? (crc >> 1) ^ CRC8_POLY_REV : (crc >> 1);
    }
    return crc;
}
...
{
    if (len < 2) return 0;
    uint8_t calc = CRC8_Calculate(data, len - 1);
    return calc == data[len - 1];
}

/*============================================
 * CRC-16/CCITT (裁判系统 + 图传整包校验, 反射实现)
 * 多项式: 0x1021 (反射 0x8408)
 * 初始值: 0xFFFF
 * 输入/输出均反射 (LSB-first)
 ============================================*/
...
uint16_t CRC16_Calculate(const uint8_t* data, uint16_t len)
{
    ...
    while (len--)
    {
        crc ^= *data++;
        for (uint8_t i = 0; i < 8; i++)
            crc = (crc & 0x0001) ? (crc >> 1) ^ CRC16_POLY_REV : (crc >> 1);
    }
    return crc;
}
...
{
    if (len < 3) return 0;
    uint16_t calc = CRC16_Calculate(data, len - 2);
    uint16_t recv = data[len - 2] | ((uint16_t)data[len - 1] << 8);
    return calc == recv;
}
